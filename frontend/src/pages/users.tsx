...
import {
  ...
  putForceUserRemoval,
  putUnblockUser
} from "../api/requests/users/users.requests";
...
const Users = () => {
  ...
  const handleRemoveUser = (id: string) => {
    putForceUserRemoval(id).then(response => {
    ...
    }).catch(err => toast.error('Failed to block the user.', {
      position: "top-right",
      autoClose: 5000,
      hideProgressBar: false,
      closeOnClick: true,
  ...
  const handleUnblockUser = (id: string) => {
    putUnblockUser(id).then(response => {
      if (response.data.success) {
        toast.success(response.data.message, {
          position: "top-right",
          autoClose: 5000,
    ...
    }).catch(err => toast.error('Failed to unblock the user.', {
      position: "top-right",
      autoClose: 5000,
      hideProgressBar: false,
      closeOnClick: true,
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'actions', headerName: 'Actions', width: 175,
      sortable: false,
      filterable: false,
      renderCell: ({row}) => {
        return (
          <div>
            ...
            {row.status === UserStatus.BLOCKED ?
              ...
              <BlockUser status={row.status} forceRemoval={row.forceRemoval} handleBlockUser={() => handleBlockUser(row.id)} />}

            <RemoveUser status={row.status} markedForRemoval={row.markedForRemoval} forceRemoval={row.forceRemoval} handleRemoveUser={() => handleRemoveUser(row.id)} />
