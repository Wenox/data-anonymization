...
\begin{itemize}
\item production \ksremark{tu i w innych listach: `,'}
\item development \ksremark{`.'}
\end{itemize}
...
\ksremark{Rozdział \ref{ch:external-specification} jest OK.}
...
This chapter summarizes the most significant internal implementation's aspects of the engineered anonymisation platform. The concepts and architecture of the system are supported by the diagrams that are rich in details. A total number of more than 400 source code files\footnote{The data as of 21st January, 2022.} somewhat describe the system's complexity hence it was not a goal for this chapter to present all the -- often repetitive or not engaging -- technical details. This chapter attempts to focus on the quality of explanation rather than the repetitiveness.

\section{System design}

Anonymisation platform implements a client-server architecture that was modernized through modularization and containerization.

\subsection{Analysis of architecture -- server}

The backend server had been designed in a modular manner since the very beginning of its implementation -- with the eventual distributed architecture preserved in mind. The server aims to separate the domain and infrastructure following the principles of clean architecture \cite{bib:clean_architecture}.
...
The modularization helps enforcing the single responsibility principle of SOLID easily \cite{bib:clean_code} -- after all the only reason for, e.g., the uploading module to change is to alter the template generation behaviour.
...
\begin{itemize}
\item \textbf{Controller}\footnote{The other synonyms recognised in the industry are \textit{ui}, \textit{presentation}, and \textit{resource}.} -- define the REST API endpoints\footnote{Although starting to become obsolete, controllers often perform the so-called server-side rendering which is a fancy name for returning the view that should be rendered to the clients.} that work as the application's entry points for the external clients.
\item \textbf{Service}\footnote{commonly known as the domain} -- define the domain functionalities of the application. In a non-trivial application, which this engineered solution is, activating one controller may activate dozens of services.
\item \textbf{Repository} -- the layer responsible for data access, i.e., persistence.
\end{itemize}

Other packages may include e.g., the mappers for data transfer objects or domain objects.

Given the fact that the names of the packages are fixed, the code is decoupled from the underlying technical purpose. An individual layer is dependent only on the adjacent layers -- for example controller layer must not be dependent on the persistence layer, meaning that the persistence layer is not aware of the controllers and vice versa.


\subsubsection{Vertical architecture}

Although a particular module has the characteristics of a layered horizontal architecture, the server divided into seven distinct modules start to have the characteristics of a vertical architecture \cite{bib:clean_architecture} where the related domain concepts are encapsulated within one package, i.e., package-by-feature. The top-level modules now tell.



Therefore this design combines the benefits of both architectural approaches.



\subsubsection{Summary}

To meet the complex and versatile needs of anonymisation platform, the server needs to implement a wide range of services. Wherever possible, the existing well-tested and secure libraries are used. Note that the actual implementations of interfaces do not follow the convention of putting \textit{-Impl} suffix to the class name -- this is considered a bad coding practice \cite{bib:clean_architecture}.

\subsection{Analysis of architecture -- client}
