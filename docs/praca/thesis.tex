...
Anonymisation is a statistical disclosure control method of a particular importance. The ultimate anonymisation goal is to deidentify the data by pruning the personal information in such a way that the relation between data subject and corresponding records is blurred.

The commonly adopted approach when attempting to anonymise the data is to first classify the data attributes as personal identifying confidential attributes and quasi-identifying attributes \cite{bib:privacy_unesco_bigdata}. The prerequisite is to understand the context of what does the data represent and by whom - in addition to how - will it be processed [13], as it is all up to this context to determine which attributes are quasi-identifiers - and what anonymisation techniques need to be used. Virtually, even the most unexpected attributes could be quasi-identifiers. Ignoring such fields when preparing for anonymisation may eventually lead to future re-identification of the data subjects, as demonstrated during the [Netflix 2005]. On the other hand, too much anonymisation strips the data out of utility.
...
\begin{table}[h]
\centering
\caption{Data before generalisation.}
...
procedure generalise(strategy, values, configuration)
  ...
  return intervals
\end{minted}
\caption{Generalisation based on distribution size.}
...
procedure generalise(strategy, values, configuration)
  ...
  return intervals
\end{minted}
\caption{Generalisation based on number of distributions.}
