...
An extensive analysis of data anonymisation subject is required to be conducted to comprehend the growing data protection needs of the society and to engineer a solution that will properly suite those needs.
...
The data anonymisation is considered to be an effective one only if both of the following criterias \ksremark{l. poj. criterion, l. mn. criteria} hold true \cite{bib:anonymization_taxa}:
\begin{itemize}
\item performed anonymisation operations are irreversible \ksremark{,}  
\item data subject re-identification is impossible or impractical \ksremark{.} 
\end{itemize}
...
\ksremark{Może lepiej: “Utility vs privacy trade-off”? Są wtedy podane oba kryteria.}
...
\ksremark{Zanim będziemy opisywać różne techniki, trzeba je wymienić, podzielić / sklasyfikować.}
...
\begin{itemize}
\item based on the size of distributions \ksremark{,}
\item based on the number of distributions \ksremark{.}
\end{itemize}

Consider the raw data of the three attributes presented in the table \ref{id:tab:generalisation_raw} \ksremark{Tu i gdzie indziej: bez „the” i ze skrótem „Tab.”, np. in Tab. \ref{id:tab:generalisation_raw}, analogicznie „in Fig. 123”.} to be put under the process of generalisation.
...
\begin{itemize}
\item fixed value \ksremark{,}
\item percentage value \ksremark{.}
\end{itemize}
...
Hashing is a deterministic masking technique \cite{bib:anonymization_for_research} which transforms data to the fixed length output. This technique is best used for unstructured data. Available implementations include SHA2 and SHA3.
...
\begin{itemize}
\item shuffle without repetitions which preserves the data distribution \ksremark{,}
\item shuffle with repetitions which distorts the data distribution \ksremark{.}
\end{itemize}
...
Anonimatron \ksremark{odnośnik do dokumentacji, strony www} is a Java tool designed for anonymisation of the databases specifically for the development purposes. The idea behind the tool is that the software developers may not be authorized to access the production data, yet may be required to reproduce, e.g., data related performance problems. Internally, anonimatron uses the concept of synonyms.
...
The people who were software enginers \ksremark{engineers} back in 1990s remember how the web had changed everything and how it dominated the marketed and the way business systems are now engineered \cite{bib:clean_architecture}.

The companies started to massively migrate their services to a cloud with the quite recent rise of the containerization and microservice architecture. Trending software concepts such as serverless or event-storming and tools such as Kubernetes — all of them are oscillating around the web, which is really the only clear and technologically available choice to build a new system. All this technological evolvement is enabled by the open source which is also continuously growing together with the web based software systems, in a feedback loop manner \cite{bib:distributed_systems}. The start of the decline in the web's growth is yet to be discovered, and likely will not emerge in the foreseeable future.
...
User must be able to initiate the process involving the removal of his account along with all the associated data, hence ensuring the GDPR compliance.

The process is started by an explicit user's requests to do so. The system should warn the user with a detailed message and a prompt to type in both the confirmation and the password. If successful, the account is marked for future removal - \ksremark{--} the account is removed by a dedicated cron-based data pruning service.
...
Docker is a virtualization software used to develop, run and manage applications in the form of containers taking advantage of Linux kernel features namespaces and cgroups. It consists of three components: a runtime, a daemon engine and an orchaestrator. 
...
\begin{itemize}
\item internally from the Docker container where Maven can be considered as an abstraction that the system administrator does not have be aware of \ksremark{;}
\item on the host system itself \ksremark{.}
\end{itemize}
...
\begin{itemize}
\item creation of a new database \ksremark{,}
\item replication of an existing databases\ksremark{,}
\item database dump restoration\ksremark{,}
\item anonymisation script execution\ksremark{,}
\item database dump to script or to archive\ksremark{.}
\end{itemize}

\subsubsection{JUnit 5}

JUnit 5 is the new and refined version of the worldwide recognized Java testing framework. It is used to drive the unit and integration tests. \ksremark{jakiś odnośnik}
...
REST Assured is an integration tests library and a HTTP client which allows sending true HTTP requests to the test server (bootstrapped by Testcontainers library). This library also offers the means to validate received HTTP responses. \ksremark{jakiś odnośnik}
...
JSON Web Tokens are required to transport the implemented access and refresh tokens necessary for authentication and authorization purposes. This popular library deals with the creation and validation of the tokens.  \ksremark{jakiś odnośnik}

\subsubsection{Bouncy Castle}

One of many cryptographic APIs offered by Bouncy Castle library is SHA 3 which internally implements Keccak-f[1600] algorithm.  \ksremark{jakiś odnośnik}

This library is used by the hashing data masking technique.

\subsubsection{Apache Commons Lang}

The sole purpose of this library is a generation of random alphabetic and alphanumeric characters for pattern masking anonymisation technique.  \ksremark{jakiś odnośnik}
...
The development was driven by git like in the case of any software project that is of a non-trivial size. A total number of 6XX \textbf{todo} commits\footnote{the data as of 21.01.2022} were pushed to the master branch. Conventional Commits specification was followed in a simplified way. The branches were deleted following the merge.
...
Furthermore, the labels with the increasing values of {XS}, {S}, {M}, {L}\ksremark{,} and {XL} were being added to all the tasks upon creation to estimate the time-to-finish complexities. The distribution of the estimations is shown in Fig. \ref{fig:img:estimations}. \textbf{todo updated pie chart}
...
The application defines several business names related to the anonymisation domain in order to concisely introduce complex technical problems. These names include:
\begin{itemize}
\item template – the user restores the database from the dump he provides to create a read-only replicable template that is used to produce worksheets
\item worksheet – a concept which represents the user's working area for configuring the anonymisation to match the exact business requirements of his interest. The worksheet is produced from the template and is used to generate the outcomes
\item summary – this is the worksheet's representation – a summary layer of the configured anonymisation that presents in one place all the information concerning the template, worksheet, operations and outcomes
\item operation – an individual anonymisation technique applied to the given attribute
\item outcome – the outcomes are generated from the worksheet after processing the anonymisation request and encapsulate the anonymisation script and anonymised dumps
\item metadata – represents the information about the template database, i.e., information concerning columns, tables, primary keys, foreign keys, number of tables and records, etc.
\end{itemize}
...
The database of a new instance of the anonymisation platform is preloaded with at least one admin account. The credentials of this initial account can be configured from the properties and must be overriden in production:
\begin{verbatim}
core:
  preloader.admin:
...
The system requires the user to verify his e-mail address to access the platform capabilities. The application allows blocking potentially malicious users. To ensure the security of users data through data protection measures, the user can request to remove his account.

\subsection{Continuous integration}
Multiple advanced continuous integrity workflows were established to follow the state of the art software development practices of ensuring the quality and security. The plans verify the state of quality, security and tests — and are triggered daily, upon creating a pull request or upon committing to the master branch.

\subsection{Containers isolation}
The platform runs as a secure network of isolated docker containers.

\subsubsection{Authentication and authorization}
Platform endpoints are gated behind the authorization logic, i.e., in order to access the platform resources the user first needs to authenticate himself by logging in to his account, and then his account is also required to have the appropriate role authority to access the particular resources such as the templates or worksheets.
...
The application pays special attention to the validation which is performed on the client's side and on the server's side in a multi-level fashion. Taking the proper validation measures prevents the potentially devastating effects of putting the application in a breaking, unsupported state.
