...
There should a validation standard established that will determine if particular anonymisation techniques are applicable together or applicable to the given data type. The techniques should be configurable.
...
Achieved through client functionalities such as intuitiveness and uniform design of the system
...
\begin{itemize}
\item Modular - server should be divided into modular architecture where each module has its own responsibiltiy. Modules have to be further divided into layers or packages. System implements client-server architecture.

\item Secure - achieved through, e.g., vulenarability scanning, token based authentication enhanced with refresh token functionality, client and server validation, containerization, role based endpoint protection.

\item Configurable- system should be highly configurable by the system administrator. The software can be separately configured on the server, client and containers side.

\item Confidential - data sent by the users should not be accessible by the admins.

\item Extensible - the system should be engineered in a way to be easily extensible with new features such as supporting new database types or new data masking techniques.

\item Adaptable - the system should be able to easily adapt to changing requirements by the means of clean code practices such as modularity and testability.

\item Performant - achieved through, e.g., using lazy loading of the relations and individual optimisations.

\item Responsive - achieved through, e.g., performant client routing.

\item Engaging - achieved by displaying skeletons, spinners and notifications.

\item Usable - the system should not be confusing to be used.

\item Testable - the system code should be written in a testable manner

\item Reliable - achieved through continuous integration testing and using well known secure libraries 

\item User experience - achieved through client functionalities such as intuitiveness and uniform design of the system

\item Measurable - the system should collect metrics about e.g. processing time of outcomes.

\item Modern - the system must not use obsolete technologies or software engineering design techniques.
\end{itemize}
